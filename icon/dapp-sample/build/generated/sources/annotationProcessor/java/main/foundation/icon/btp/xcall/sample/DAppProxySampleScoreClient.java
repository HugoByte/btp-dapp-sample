...
public final class DAppProxySampleScoreClient extends DefaultScoreClient implements CallServiceReceiver {
  ...
  public void sendMessage(Consumer<TransactionResult> consumerFunc, String _to, BigInteger _type,
      byte[] _data, @Optional byte[] _rollback) {
    Map<String,Object> params = new HashMap<>();
    params.put("_to",_to);
    params.put("_type",_type);
    params.put("_data",_data);
  ...
  public void sendMessage(BigInteger valueForPayable, String _to, BigInteger _type, byte[] _data,
  ...
  public void sendMessage(Consumer<TransactionResult> consumerFunc, BigInteger valueForPayable,
      String _to, BigInteger _type, byte[] _data, @Optional byte[] _rollback) {
    Map<String,Object> params = new HashMap<>();
    params.put("_to",_to);
    params.put("_type",_type);
    params.put("_data",_data);
  ...
  public Consumer<TransactionResult> MessageEend(Consumer<List<MessageEend>> consumerFunc,
      Predicate<MessageEend> filter) {
    return (txr) -> consumerFunc.accept(MessageEend.eventLogs(txr, this.address, filter));
  }
  ...
  public static class MessageEend {
    public static final String SIGNATURE = "MessageEend(str)";

    public static final int INDEXED = 0;

    private final String _from;

    public MessageEend(TransactionResult.EventLog el) {
      List<String> indexed = el.getIndexed();
      List<String> data = el.getData();
      this._from=data.get(0);
    }
    ...
    public static List<MessageEend> eventLogs(TransactionResult txr, Address address,
        Predicate<MessageEend> filter) {
      return DefaultScoreClient.eventLogs(txr, SIGNATURE, address, MessageEend::new, filter);
    }
  }
